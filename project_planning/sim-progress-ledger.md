...
| SIM.1 Mock Aggregator | not started | 53 | — | — | `/sim` project skeleton, `sim/checksum.py`, `MockAggregator`. |
| SIM.2 Mock Listener behaviour | not started | 54 | — | — | `MockListener`, spec 6.5 liveness, wake declarations over the modelled local link. |
| SIM.3 Scenario scripting | not started | 55 | — | — | Behaviour registry + TOML files; six catalogued scenarios, each asserting the platform's reaction. |
