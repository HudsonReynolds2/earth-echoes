...
> **Addendum PHASE2-4-02 (2026-08-04, ref project-changes #18):** Shipped with three concretizations (DECISIONS D55-D56): apply gains a `level` control — `"target"` writes onto each matched entity, a named level writes once at the single common ancestor (422 naming candidates on a split; organization level needs an org-wide grant); revisions are per-DEVICE only (aggregator/listener, the spec 6.1/7.2 addressees) with marker-bearing snapshots composed per spec 5.4; and the spec-13 revisions read routes, unassigned in this document, landed here because E2.8's acceptance needs them. Preview paginates; streaming is recorded as an E8.2 scale seam.
