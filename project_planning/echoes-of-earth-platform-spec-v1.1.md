...
> **Addendum SPEC-5-01 (2026-08-13, ref project-changes #32):** the catalog gains a **thirty-eighth** key, `services.credentials_generation` (`int`, default `0`, resolves at Deployment level, not a secret), added by E5.11 on the owner's explicit decision; the table above is left as written because planning documents are append-only, and this addendum is the thirty-eighth row. Section 16.3 requires that "rotation is a config revision, not a manual redistribution", and it was not: a device's desired snapshot carries secret MARKERS rather than plaintext (Sections 5.4 and 8), and a marker is a SecretStore NAME — the identical string before and after a rotation. Measured, rotating every credential a deployment has minted ZERO revisions and told no device anything, while rotating to a different hostname minted one per Aggregator — so the projection path was working and there was simply nothing to say. The counter is the one non-secret thing a rotation changes, and it is a count rather than a timestamp because two renders of one generation must be byte-identical and a clock is not. Like the other Section 16 keys it is write-restricted, so the onboarding flow writes it and operators are refused it, and Section 5.4 keeps it out of Listener-bound files. Rationale in DECISIONS.md D146; the frozen merge-engine checksum it moves is D149.
...
> **Addendum SPEC-17-01 (2026-08-11, ref project-changes #37):** Item 14 is **decided and closed: v1 requires dynsec.** The platform mints per-device broker credentials only through the Mosquitto dynamic security API, and a broker that fails the Section 16.2 dynsec probe cannot reach `verified` — which by Section 16.5 blocks provisioning-bundle generation, since the bootstrap block would otherwise embed credentials that do not exist. Section 16.4's sentence "Against a Path A broker without dynsec, the platform generates the credential pair and presents it to the operator to install on the broker manually, and the bundle is held until the operator confirms" is therefore **superseded**: no manual path and no held-bundle state ships in v1. The probe reports three verdicts rather than two — `available`, `denied` (plugin present, platform account not an admin), and `absent` — because the last two have different remedies. Rationale in DECISIONS.md D116.
