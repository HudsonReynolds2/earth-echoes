...
- **EPIC E5 IS COMPLETE.** C5 is green at gate-62 and the PR is open. What is deliberately NOT
  built, so nobody "finishes" it by mistake: periodic service re-checks (D133 — closed, do not
  add a scheduler), the spec 16.5 gate's *enforcement* (D145 — E4.3's, E5 only reports it), and
  a "does a stack exist" endpoint (D144 — fixed choice 7 stores nothing that would say so).
- **A RUNTIME DATA FILE GOES UNDER `backend/app/`, and this cost a 500 to learn (D146).** The
  ...
  exists**. That is D132's shape a second time in one epic: an assertion true of the developer's
...
- **The E5 walkthrough earned its keep on its first run**, which is the third time in this epic
  that running the artifact beat inspecting it (the E5.10 keystone's four defects, D139's sweep,
  now D146). If a later epic is tempted to ship a walkthrough without hand-running it, this is
  the argument against.
- **The services field table is a mirror and the parity test is what keeps it one (D140).** Add
