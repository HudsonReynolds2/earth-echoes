...
5. Open the API at `http://localhost:8000` and the frontend at `http://localhost:5173`.
6. For backend work outside containers: `cd backend` then `uv sync`.
7. For frontend work outside containers: `cd frontend` then `npm ci`, then `npm run dev`.
8. Before finishing any task, run the gate: `./gate.ps1` (Windows) or `make gate` (POSIX);
   the entire suite must pass (see `CLAUDE.md`, rule R0).
9. Read `docs/INTERFACES.md` and `docs/DECISIONS.md` before changing anything they cover.
